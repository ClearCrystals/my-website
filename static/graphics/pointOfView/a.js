function setup() {
    ...
    // Add the drawCircle function here
     // Add the drawCircle function here
     function drawCircle(color, Tx, radius) {
        context.beginPath();
        context.arc(Tx[0], Tx[1], radius, 0, 2 * Math.PI, false);
        context.fillStyle = color;
        context.fill();
    }

    // Add the drawEarthAndSun function here
    function drawEarthAndSun(earthOrbitRadius, earthRadius, sunRadius, TxU, rotate = false) {
        // Calculate the position of the center of the canvas
        var canvasCenterX = observerCanvas.width / 2;
        var canvasCenterY = observerCanvas.height / 2;
    
        // Adjust the Sun's position to be the center of the canvas
        var sunPosition = [canvasCenterX, canvasCenterY, 0]; // Z is 0 for 2D canvas
    
        // Calculate Earth's position in its orbit, adjusted for canvas center
        var earthX = canvasCenterX + (Math.cos(rotate ? viewAngle : tParam) * earthOrbitRadius);
        var earthY = canvasCenterY + (Math.sin(rotate ? viewAngle : tParam) * earthOrbitRadius);
    
        // Draw the Sun at the center
        drawCircle('yellow', sunPosition, sunRadius);
    
        // Draw the Earth in orbit
        drawCircle('blue', [earthX, earthY], earthRadius);
    }
    ...
    // Define the Earth's orbit radius and the sizes of the Earth and Sun
    var earthOrbitRadius = 100;
    var moonOrbitRadius = 15; // smaller orbit for the moon
    var earthRadius = 5;
    var moonRadius = 2; // smaller size for the moon
    var sunRadius = 20;

    // Calculate Earth's position in its orbit
    var earthX = Math.cos(tParam) * earthOrbitRadius;
    var earthY = Math.sin(tParam) * earthOrbitRadius;

    // Calculate the Moon's position in its orbit around the Earth
    var moonAngle = tParam * 12; // The moon orbits faster than the Earth
    var moonX = earthX + Math.cos(moonAngle) * moonOrbitRadius;
    var moonY = earthY + Math.sin(moonAngle) * moonOrbitRadius;

    // Draw the solar system for the observer's view
    context = observerContext;
    drawCircle('yellow', [200, 200], sunRadius); // Draw the Sun at the center of the canvas
    drawCircle('blue', [200 + earthX, 200 + earthY], earthRadius); // Draw the Earth
    drawCircle('gray', [200 + moonX, 200 + moonY], moonRadius); // Draw the Moon

    // ... existing code to draw axes and trajectories for observer ...

    // Draw the solar system for the camera's view with rotation
    context = cameraContext;
    var rotatedEarthX = Math.cos(viewAngle) * earthX - Math.sin(viewAngle) * earthY;
    var rotatedEarthY = Math.sin(viewAngle) * earthX + Math.cos(viewAngle) * earthY;
    var rotatedMoonX = Math.cos(viewAngle) * moonX - Math.sin(viewAngle) * moonY;
    var rotatedMoonY = Math.sin(viewAngle) * moonX + Math.cos(viewAngle) * moonY;

    drawCircle('yellow', [200, 200], sunRadius); // Draw the Sun at the center of the canvas
    drawCircle('blue', [200 + rotatedEarthX, 200 + rotatedEarthY], earthRadius); // Draw the Earth with rotation
    drawCircle('gray', [200 + rotatedMoonX, 200 + rotatedMoonY], moonRadius); // Draw the Moon with rotation


    // Draw the following in the Camera window
    context = cameraContext;
    //drawEarthAndSun(100, 5, 20, tVP_PROJ_VIEW_Observer);
    ...
    context = observerContext;
    //drawEarthAndSun(100, 5, 20, tVP_PROJ_VIEW_Camera, true); // Rotate for camera view
